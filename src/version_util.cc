		       ...
		       rapidjson::Document &version_manifest)
{
	...
	if (version_data.IsNull()) {
		BREAD_LOG(
			"The specified version ({}) is not present in the version manifest\n",
			version);
	...
	BREAD_LOG("Version {} is of type {} and is defined at"
		  " {}.\n",
		  version, version_data["type"].GetString(),
		  version_data["url"].GetString());
	try {
		fs::create_directory(root_dir + "/versions/" + version);
	} catch (std::exception e) {
		BREAD_LOG("Failed to create directory {}/versions/{}: {}",
			  root_dir, version, e.what());
		return rapidjson::Document();
	}

	/* Get the definition of this version */
	version_raw = get_file(version_data["url"].GetString(),
			       fmt::format("{}/versions/{}/version.json",
					   root_dir, version),
			       true);
